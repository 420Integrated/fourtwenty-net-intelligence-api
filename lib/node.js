...
var WS_SECRET = process.env.WS_SECRET || "fourtwenty-net-stats-has-a-secret";
...
function Node ()
{
	this.info = {
		...
		protocol: null,
		api: null,
		port: (process.env.LISTENING_PORT || 13013),
		os: os.platform(),
		os_v: os.release(),
...
Node.prototype.startWeb3Connection = function()
{
	...
	web3 = new Web3();
	web3.setProvider(new web3.providers.HttpProvider('http://' + (process.env.RPC_HOST || 'localhost') + ':' + (process.env.RPC_PORT || '6174')));
...
Node.prototype.getInfo = function()
{
	...
	try {
		this.info.coinbase = web3.fourtwenty.coinbase;
		this.info.node = web3.version.node;
		this.info.net = web3.version.network;
		this.info.protocol = web3.toDecimal(web3.version.fourtwentycoin);
...
Node.prototype.getLatestBlock = function ()
{
	...
	if(this._web3)
	{
		...
		web3.fourtwenty.getBlock('latest', false, function(error, result) {
...
Node.prototype.getStats = function(forced)
{
	...
	if (this._web3 && (lastFetchAgo >= UPDATE_INTERVAL || forced === true))
	{
		...
		async.parallel({
			...
			{
				web3.fourtwenty.getMining(callback);
			},
			hashrate: function (callback)
			{
				web3.fourtwenty.getHashrate(callback);
			},
			smokePrice: function (callback)
			{
				web3.fourtwenty.getSmokePrice(callback);
			},
			syncing: function (callback)
			{
				web3.fourtwenty.getSyncing(callback);
			}
		},
		function (err, results)
		{
			...
			if(results.peers !== null)
			{
				...
				self.stats.smokePrice = results.smokePrice.toString(10);
...
Node.prototype.getPending = function()
{
	...
	if (this._web3)
	{
		...
		web3.fourtwenty.getBlockTransactionCount('pending', function (err, pending)
		{
			if (err) {
...
Node.prototype.getHistory = function (range)
{
	...
	{
		web3.fourtwenty.getBlock(number, false, callback);
	},
	function (err, results)
	{
		if (err) {
...
Node.prototype.prepareStats = function ()
{
	return {
		...
		stats: {
			...
			smokePrice: this.stats.smokePrice,
...
Node.prototype.setWatches = function()
{
	...
	web3.fourtwenty.isSyncing(function(error, sync) {
		if(!error) {
			if(sync === true) {
				web3.reset(true);
...
Node.prototype.setFilters = function()
{
	...
	this._latestQueue = async.queue(function (hash, callback)
	{
		...
		web3.fourtwenty.getBlock(hash, false, function (error, result)
		{
			self.validateLatestBlock(error, result, timeString);
	...
	try {
		this.chainFilter = web3.fourtwenty.filter('latest');
		this.chainFilter.watch( function (err, hash)
		{
			...
			if(hash === null)
			{
				hash = web3.fourtwenty.blockNumber;
			}
	...
	try {
		this.pendingFilter = web3.fourtwenty.filter('pending');
